...
TEST(CollisionTests, IdenticalPositionsDoNotApplyAmbiguousImpulse) {
  ...
  RigidBody body2(1.0f, 1.0f);
  body2.position = Vec3(0.f, 0.f, 0.f);
  body2.velocity = Vec3(-1.f, 0.f, 0.f);
  world.addBody(&body2);
  ...
  EXPECT_FLOAT_EQ(body1.velocity.x, 1.f);
  EXPECT_FLOAT_EQ(body1.velocity.y, 0.f);
  EXPECT_FLOAT_EQ(body1.velocity.z, 0.f);
  EXPECT_FLOAT_EQ(body2.velocity.x, -1.f);
  EXPECT_FLOAT_EQ(body2.velocity.y, 0.f);
  EXPECT_FLOAT_EQ(body2.velocity.z, 0.f);
}
